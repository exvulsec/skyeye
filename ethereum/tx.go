...
func NewTransactionExecutor(blockExecutor BlockExecutor, chain, table, openapi, redisVersion string, workers, batchSize int, nonce uint64, isCreationContract bool) Executor {
	return &transactionExecutor{
		blockExecutor:      blockExecutor,
		workers:            workers,
		batchSize:          batchSize,
		isCreationContract: isCreationContract,
		exporters:          exporter.NewTransactionExporters(chain, table, openapi, redisVersion, nonce),
	}
}
